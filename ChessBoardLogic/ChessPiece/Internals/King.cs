...
internal class King : ChessPiece
{
    ...
    public override IEnumerable<ChessMove> PossibleMoveList()
    {
        ...
        {
            ChessMove move = new(Location.ColLoc, Location.RowLoc, (ColPos)iEndCol, (RowPos)iEndRow);
        ...
        if (iEndRow < 9)
        {
            ChessMove move = new(Location.ColLoc, Location.RowLoc, Location.ColLoc, (RowPos)iEndRow);
        ...
        if (iEndCol > 0)
        {
            ChessMove move = new(Location.ColLoc, Location.RowLoc, (ColPos)iEndCol, Location.RowLoc);
        ...
        if (InitialPosition)
        {
            ChessPieceLocation right = new(ColPos.G, Location.RowLoc);
            if (Board.CanMove(Location, right))
            {
                yield return new ChessMove(Location, right);
            }

            ChessPieceLocation left = new(ColPos.C, Location.RowLoc);
    ...
    {
        ChessPieceColor opponentColor = PieceColor == ChessPieceColor.Black ? ChessPieceColor.White : ChessPieceColor.Black;
