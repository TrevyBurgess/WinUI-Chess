...
namespace TrevyBurgess.Games.TrevyChess.ChessBoardLogic
{
    ...
    public abstract class ChessPiece
    {
        ...
        {
            foreach (ChessPiece possibleDefender in Board.GetLivePieces(this.PieceColor))
            {
                // don't include this piece
                if (possibleDefender == this)
                    continue;

                if (possibleDefender.CanCover(this.Location))
        ...
        {
            ChessPieceColor attackerColor = this.PieceColor == ChessPieceColor.Black ? ChessPieceColor.White : ChessPieceColor.Black;

            foreach (ChessPiece possibleAttacker in Board.GetLivePieces(attackerColor))
            {
                if (possibleAttacker.CanMove(this.Location))
        ...
        {
            if (moveFreeStyle || this.CanMove(newPosition))
            {
                // Initial position
                ...
                if (endPiece == null)
                {
                    ...
                    if (this.PieceColor == ChessPieceColor.White && newPosition.RowLoc == RowPos.R6)
                    {
                    ...
                    else if (this.PieceColor == ChessPieceColor.Black && newPosition.RowLoc == RowPos.R3)
                    {
        ...
        protected bool CanMoveHorVert(ChessPieceLocation newPosition)
        {
            ...
            if (Board[newPosition.ColLoc, newPosition.RowLoc] != null &&
                Board[newPosition.ColLoc, newPosition.RowLoc].PieceColor == this.PieceColor)
                return false;
        ...
        protected bool CanMoveHorVert()
        {
            ...
            if (col > 1)
            {
                ChessPieceLocation newPosition = new ChessPieceLocation((ColPos)(col - 1), (RowPos)(row));
            ...
            if (row > 1)
            {
                ChessPieceLocation newPosition = new ChessPieceLocation((ColPos)(col), (RowPos)(row - 1));
                if (CanMoveHorVert(newPosition))
                    return true;
            }
